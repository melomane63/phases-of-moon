...
export default class PhasesOfMoonExtension extends Extension {
    ...
    /**
     * Called when display-related settings change
     * Updates the indicator to reflect the new values
     */
    _onSettingsChanged() {
        const reversed = this._settings.get_boolean('show-reversed-moon-phase');
        const southern = this._settings.get_boolean('southern-hemisphere');

        if (this._indicator && typeof this._indicator._updateMoonPhase === 'function') {
            this._indicator._updateMoonPhase(reversed, southern);
        }
    }
}
